...
parameters {
  ...
  // fixed interaction between display and tdd
  real b_tdd_5_X_h;
  real b_tdd_9_X_h;
  real b_tdd_14_X_h;
  ...
  real<lower=0> sigma_btdd5_s;
  
  // std dev of effect of tdd=9
  real<lower=0> sigma_btdd9_s;
  
  // std dev of effect of tdd=14
  real<lower=0> sigma_btdd14_s;
  
  // std dev of effect of tdd5 x td
  real<lower=0> sigma_btdd5xtd_s;
  
  // std dev of effect of tdd9 x td
  real<lower=0> sigma_btdd9xtd_s;
  
  // std dev of effect of tdd14 x td
  real<lower=0> sigma_btdd14xtd_s;
  
  // std dev of effect of tdd5 x h
  real<lower=0> sigma_btdd5xh_s;
  
  // std dev of effect of tdd9 x h
  real<lower=0> sigma_btdd9xh_s;
  
  // std dev of effect of tdd14 x h
  real<lower=0> sigma_btdd14xh_s;
  ...
  // random interaction between display and tdd
  vector[n_subs] b_tdd_5_X_h_s;
  vector[n_subs] b_tdd_9_X_h_s;
  vector[n_subs] b_tdd_14_X_h_s;
...
model {
  ...
  b_tdd_5_X_h ~ normal(0,2.5);
  b_tdd_9_X_h ~ normal(0,2.5);
  b_tdd_14_X_h ~ normal(0,2.5);
  ...
  sigma_btdd5xh_s ~ lognormal(0,.5);
  sigma_btdd9xh_s ~ lognormal(0,.5);
  sigma_btdd14xh_s ~ lognormal(0,.5);
  ...
  b_tdd_5_X_h_s ~ normal(0, sigma_btdd5xh_s);
  b_tdd_9_X_h_s ~ normal(0, sigma_btdd9xh_s);
  b_tdd_14_X_h_s ~ normal(0, sigma_btdd14xh_s);
  ...
  for(i in 1:n_trials) {
    discrim[i] ~ bernoulli_logit( 
      ...
      (b_tdd_5_X_h + b_tdd_5_X_h_s[sub_n_new[i]])*tdd_5[i]*display[i]+ // interaction between tdd and display
      (b_tdd_9_X_h + b_tdd_9_X_h_s[sub_n_new[i]])*tdd_9[i]*display[i]+
      (b_tdd_14_X_h + b_tdd_14_X_h_s[sub_n_new[i]])*tdd_14[i]*display[i]
  );
}
...
generated quantities{
  ...
  for(i in 1:n_trials_unique){
    p[i] = inv_logit((b_0+b_0_s[sub_n_new_unique[i]]) +  // intercept
                     ...
                     (b_tdd_5_X_h + b_tdd_5_X_h_s[sub_n_new_unique[i]])*tdd_5_unique[i]*display_unique[i]+
                     (b_tdd_9_X_h + b_tdd_9_X_h_s[sub_n_new_unique[i]])*tdd_9_unique[i]*display_unique[i]+
                     (b_tdd_14_X_h + b_tdd_14_X_h_s[sub_n_new_unique[i]])*tdd_14_unique[i]*display_unique[i]
                     );
  }
}
