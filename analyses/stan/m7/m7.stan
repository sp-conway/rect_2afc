// THIS MODEL REMOVES THE RANDOM INTERACTION
...
parameters {
  ...
  // std dev. of random subject level effects
...
model {
  ...
  //sigma_b_tdd_s ~ cauchy(0,1);
  //sigma_b_h_s ~ cauchy(0,1);
  
  // random effects - coefficients
  b_0_s ~ normal(0,sigma_b_0_s);
  b_w_s ~ normal(0,sigma_b_s);
  b_td_s ~ normal(0,sigma_b_s);
  b_h_s ~ normal(0,sigma_b_s);
  b_tdd_5_s ~ normal(0,sigma_b_s);
  b_tdd_9_s ~ normal(0,sigma_b_s);
  b_tdd_14_s ~ normal(0,sigma_b_s);
  ...
  for(i in 1:n_trials) {
    discrim[i] ~ bernoulli_logit( 
      ...
      (b_tdd_5_X_td ) * tdd_5[i] * probe[i] +  // effect of tdd=5 & probe=td
      (b_tdd_9_X_td ) * tdd_9[i] * probe[i] + // effect of tdd=9 & probe=td
      (b_tdd_14_X_td ) * tdd_14[i] * probe[i]  // effect of tdd=14 & probe=td
  );
}
...
generated quantities{
  ...
  for(i in 1:n_trials_unique){
    p[i] = inv_logit((b_0+b_0_s[sub_n_new_unique[i]]) +  // intercept
                     ...
                     (b_tdd_5_X_td )*tdd_5_unique[i]*probe_unique[i] +  // effect of tdd=5 & probe=td
                     (b_tdd_9_X_td)*tdd_9_unique[i]*probe_unique[i] + // effect of tdd=9 & probe=td
                     (b_tdd_14_X_td )*tdd_14_unique[i]*probe_unique[i] );
  }
}
